...
RSpec.describe Comment, type: :model do
  ...
  it "コメントが空ならバリデーションが通らない" do
    comment = Comment.new(content: "", user_id: @second_user.id, stroll_id: @stroll.id)
  ...
  it "stroll_idが空ならバリデーションが通らない" do
    comment = Comment.new(content: "chiba", user_id: @second_user.id, stroll_id: "")
    expect(comment).not_to be_valid
  end
  it "コメントとuser_idとstroll_idが入力されてたらバリデーションが通る" do
    comment = Comment.new(content: "chiba", user_id: @second_user.id, stroll_id: @stroll.id)
